...
import {
	AdditionalPickupRequestHandfreeDto,
} from 'requestServices/dto/additional-pickup-request-handfree.dto'
...
export class RequestServicesService {
	...
	async additionalPickupRequestHandfree(id: string, body: AdditionalPickupRequestHandfreeDto) {
		if (!isUuid(id)) {
			throw new HttpException('Invalid service ID format', HttpStatus.BAD_REQUEST)
		}
		const service = await this.findOneColumn(id)
		if (!service || service.type !== 'Handsfree Request') {
			throw new HttpException('Service not found or not a Handsfree Request', HttpStatus.BAD_REQUEST)
		}
		if (service.state !== 'Bags Collected') {
			throw new HttpException('Service is not in a state that allows additional pickup requests', HttpStatus.BAD_REQUEST)
		}

		const state = 'Additional Pickup Requested'
		service.state = state
		service.metadata = service.metadata || {}
		if (!Array.isArray(service.metadata.additionalPickupRequest)) {
			service.metadata.additionalPickupRequest = []
		}
		service.metadata.additionalPickupRequest.push(body)

		await this.requestServicesRepository.update(id, { state, metadata: service.metadata })
		await this.elasticService.updateDocument('services', id, {
			state,
